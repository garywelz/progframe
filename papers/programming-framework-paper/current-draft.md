...
```mermaid
%%{init:{'theme':'base','themeVariables':{'background':'#ffffff','clusterBkg':'#ffffff','clusterBorder':'#cccccc','mainBkg':'#ffffff','primaryColor':'#ffffff','secondaryColor':'#ffffff','tertiaryColor':'#ffffff'}}}%%
flowchart TB
  subgraph STACK[" "]
    direction TB
...
style LG_V fill:#b197fc,color:#fff
```

**Figure 1:** Minimal walkthrough with all five semantic colours plus an **in-chart legend**. **`{ … }` Diamonds** denote branch predicates coloured **Blue** like other intermediates (§3.2)—**shape**, not Violet fill, distinguishes them from outputs. Mutual exclusion at a branch (“Yes”/“No”) is standard diamond syntax; **AND** combinations use **chained** predicates, convergent incoming edges before a downstream step, or speciality overlays—for example optional AND-join patterns in specialised corpora (cf. mathematics proofs pilot). **Inclusive OR** (“any of several predecessors”) uses **multiple incoming edges**, not another legend colour. Hex values: Red #ff6b6b; Yellow #ffd43b; Green #51cf66; Blue #74c0fc; Violet #b197fc. Figures that appear “light purple” in companions are usually **Violet outputs**, topology-specific hues, or a non–§3.2 palette (§4.1)—not substitutes for predicate Blue.
