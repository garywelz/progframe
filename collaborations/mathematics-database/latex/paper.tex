...
\author{Gary Welz \\[0.6em]{\normalsize Researcher, New Media Lab, CUNY Graduate Center \\[0.35em]Email: \texttt{gwelz@gc.cuny.edu} \\[0.35em]ORCID: \url{https://orcid.org/0009-0005-7806-0892}}}
\date{}
...
5-color scheme as in the Mathematics Database table: Red = input,
Lavender = decision, Green = operation, Violet = output.}
...
\bottomrule\noalign{}
\endlastfoot
Source & Amber (\texttt{\#f2c879}) & The theorem, proposition, or claim
being proved \\
Assumption & Violet (\texttt{\#c084fc}) & A temporary assumption (e.g.,
for contradiction or induction) \\
Construction & Moss (\texttt{\#8fbc5a}) & An object explicitly
constructed in the proof \\
Assertion & Steel (\texttt{\#8ecae6}) & A claim that follows from prior
steps \\
Inference & Copper (\texttt{\#f4a261}) & A logical inference rule or
proof step \\
Algorithm Capsule & Indigo (\texttt{\#818cf8}) & An embedded algorithmic
substructure within the proof \\
Contradiction & Crimson (\texttt{\#ef4444}) & A contradiction reached
(in proof by contradiction) \\
Conclusion & Teal (\texttt{\#2dd4bf}) & The final conclusion
establishing the theorem \\
\end{longtable}

Node fills use the same muted proof-role palette as the live proof-graph
pages in the Mathematics Database (\texttt{proof-graphs/}). Algorithmic
flowcharts and axiomatic dependency graphs use the GLMP 5-color scheme
shown in the database table viewer.
...
infinitude-of-primes argument. Colors match the Mathematics Database
proof-graph palette: Amber = source, Violet = assumption, Indigo =
algorithm capsule, Steel = assertion, Copper = inference, Crimson =
contradiction, Teal = conclusion.
